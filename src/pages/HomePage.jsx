  ...
  return (
    <Link to={to}>
      <motion.div
        ...
        whileTap={{ scale: 0.97 }}
        className="flex flex-col items-center gap-2 bg-slate-800/60 border border-slate-700 rounded-2xl p-4 cursor-pointer hover:bg-slate-700/60 transition-colors"
      >
...
function DeckCard({ deck, t }) {
  ...
  return (
    <Link to={`/decks/${deck.id}/study`}>
      <motion.div
        whileHover={{ x: 2 }}
        className="flex items-center gap-3 bg-slate-800/50 border border-slate-700 rounded-xl p-4 hover:bg-slate-700/50 transition-colors"
      >
        <div className="flex-1 min-w-0">
          <div className="font-semibold truncate">{deck.name}</div>
          <div className="text-xs text-slate-400 mt-0.5">{deck.cards.length} {t('home.cards')}</div>
        </div>
        {due > 0 && (
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="text-sm text-slate-400">{t('home.welcome')}</div>
